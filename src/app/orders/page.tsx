...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    const userId = payload.id || payload._id; //https://ecommerce.routemisr.com/api/v1/orders/user/6407cf6f515bdcf347c09f17
              ...
              <div className="p-4 flex items-center gap-4">
                <div className="relative w-16 h-16 flex-shrink-0">
                  {order.cartItems.slice(0, 2).map((item: any, i: number) => (
                    <img key={item._id} src={item.product?.imageCover} alt={item.product?.title}
